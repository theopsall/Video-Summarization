...
def file_shorten(input_file, timestamps_file, output_file):
    ...
    if os.path.isfile(output_file):
        print(
            "Output file {0} already exists!\n Please change output file name or delete the".format(
                output_file
            )
        )
        print("Exiting")
    ...
    for i in range(total_timestamps):
        ...
        cmd = "ffmpeg -i '{0}' -ss {1} -to {2} -c copy {3}".format(
            input_file,
    ...
    cmd = "ffmpeg -f concat -i {0} -c copy {1}".format(
        "temporary_list.txt", "'{0}'".format(output_file)
    )
    sys.stdout = sys.__stdout__
    os.system(cmd)
...
def dir_shorten(input_directory, timestamps_directory, output_directory):
    ...
    if not os.path.isdir(output_directory):
        print(
            "Output {0} directory not found! \n Resolving by creating the directory".format(
                output_directory
            )
        )
    ...
    for video_file, timestamp in zip(videos, timestamps):
        ...
        output_file = os.path.join(
            output_folder, "short_{0}".format(video_file.split(os.sep)[-1])
        )
        print(output_file)
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="A demonstration script for video shorting"
    )
    tasks = parser.add_subparsers(
        title="subcommands", description="available tasks", dest="task", metavar=""
    )

    single_file = tasks.add_parser("fileShorten", help="Shorten a video file")
    ...
    single_file.add_argument(
        "-o", "--output", required=True, help="Output directory for shorten video file"
    )

    video_dir = tasks.add_parser(
        "dirShorten", help="Shorten all videos on the directory"
    )
    video_dir.add_argument(
        "-i", "--input", required=True, help="Videos input directory"
    )
    video_dir.add_argument("-l", "--labels", required=True, help="Timestamps directory")
    video_dir.add_argument(
        "-o", "--output", required=True, help="Output directory for shorten videos"
    )
